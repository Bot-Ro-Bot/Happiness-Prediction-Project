...
for(filename in files){
  ...
  # read csv file
  year_data = read.csv(filename,stringsAsFactors = F) # F to explore raw dataaset
...
```{r}
lin_reg = lin_reg1 = lm(train_response~., data=train_features)
summary(lin_reg)
...
lin_reg = lin_reg2 = lm(train_response~., data=train_features)
...
lin_reg = lin_reg3=  lm(train_response~., data=train_features)
...
# | Lasso Regression     |  0.45     |  0.54    |
# | Ridge Regression     |  0.45     |  0.55    |
# | KNN                  |  0.20     |  0.17    |
```
